...
class DonationTest extends TestCase
{
    ...
    {
        $project = $this->donatableProject(['budget' => 1000]);
        $donor = $this->makeUser();
        $admin = $this->makeUser();
    ...
    public function test_donation_statistics_are_calculated_correctly(): void
    {
        ...
        $this->postJson("/api/admin/project/{$project->id}/donations", ['donor_user_id' => $donorA->id, 'amount' => 300], $adminHeaders)->assertStatus(201);
        $this->postJson("/api/admin/project/{$project->id}/donations", ['donor_user_id' => $donorB->id, 'amount' => 400], $adminHeaders)->assertStatus(201);
        ...
        $this->assertEqualsWithDelta(700.0, $data['total_donated'], 0.001);
        $this->assertEqualsWithDelta(1000.0, $data['donation_target'], 0.001);
        $this->assertEqualsWithDelta(300.0, $data['remaining_amount'], 0.001);
    ...
    public function test_donation_percentage_handles_zero_target_without_division_by_zero(): void
    {
        ...
        $this->postJson("/api/admin/project/{$project->id}/donations", ['donor_user_id' => $donor->id, 'amount' => 100], $adminHeaders)->assertStatus(201);
    ...
    public function test_top_donors_are_ranked_by_total_donated_with_correct_counts(): void
    {
        ...
        $this->postJson("/api/admin/project/{$project->id}/donations", ['donor_user_id' => $donorA->id, 'amount' => 100], $adminHeaders)->assertStatus(201);
        $this->postJson("/api/admin/project/{$project->id}/donations", ['donor_user_id' => $donorA->id, 'amount' => 50], $adminHeaders)->assertStatus(201);
        $this->postJson("/api/admin/project/{$project->id}/donations", ['donor_user_id' => $donorB->id, 'amount' => 500], $adminHeaders)->assertStatus(201);
        ...
        $this->assertEqualsWithDelta(500.0, $donors[0]['total_donated'], 0.001);
        $this->assertSame(1, $donors[0]['donation_count']);

        $this->assertSame($donorA->id, $donors[1]['user_id']);
        $this->assertEqualsWithDelta(150.0, $donors[1]['total_donated'], 0.001);
    ...
    public function test_donation_records_can_be_listed_for_a_project(): void
    {
        ...
        $this->assertEquals(100, $rows[0]['payment']);
